...
| 4 | Shared safe diagnostics and observability | Diag 0.6, Status 1.3, retained D610h..D613h record, operations 24h/25h/27h, generated memory map | Complete; stock/JukuNet mechanisms are self-contained and destructive live-CP/M tests are explicitly rejected. |
...
- **Mandatory ROM menu/local disk boot:** intentionally absent. Network boot is
  the product. C8 retains its concealed S21-bit-0 recovery wait; the separately
  named C9 simulator/HDL candidate boots unconditionally and reserves the
  freed bit. Its bounded host transport, rather than that policy alone,
  justified the new candidate. Physical programming remains unauthorized.
