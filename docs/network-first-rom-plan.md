...
| 6. Move services / relink | In progress | The CP/M image uses resident serial, keyboard, and the 1,191-byte console/font with a 119-byte copied helper. ABI cosim checks a shifted `T` and exact 9,600-byte `Z`/cursor frame; full cosim types `DIR`/`DIAG CPU` and proves byte-exact screen parity with the RAM console. The 897-byte ROM binding replaces 2,463 baseline platform/keyboard bytes, saving 1,566 linked bytes. Fixed NetDisk origins still leave a 4,080-byte initialized span, so whole NetDisk migration and final relinking remain before a TPA claim. |
