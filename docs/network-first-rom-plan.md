...
| 6. Move services / relink | In progress | A dedicated CP/M image validates ABI 1.0 and uses resident serial plus the shared polled keyboard. Cosim injects a shifted `T` at the ABI level, then types all 13 characters for `DIR` and `DIAG CPU` through the resident scanner in the real system. Removing the 331-byte RAM module while adding bindings saves 284 linked code bytes; fixed placement still leaves a 4,080-byte initialized span, so no TPA gain is claimed yet. Console/font, then whole NetDisk/bulk services and final relinking remain. |
