...
| 6. Move services / relink | In progress | A dedicated CP/M image validates ABI 1.0, then uses resident `JCGINIT` and `JCGSERINIT` for 19,200/8O1 initialization. Cosim proves the ready flag and resident serial state before `A>`, `DIR`, and `DIAG CPU`. The adapter changes from 2,132 to 2,130 linked bytes, so no TPA gain is claimed yet. Keyboard, console/font, then whole NetDisk/bulk services remain. |
