...
post-baseline distribution plan. `tools/extract_cpm3_utilities.py` refuses an
archive, member, size, or digest mismatch before exposing a `.COM` file to the
volume builder. The archives are inputs, not permission to add every contained
binary to the Juku distribution.
